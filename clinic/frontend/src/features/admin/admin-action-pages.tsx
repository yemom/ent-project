...
import { Activity, ArrowLeft, CheckCircle2, Shield, UserRoundPlus, Mail, Phone, BadgeCheck, Search, Upload, AlertTriangle } from 'lucide-react';
...
function ActionLayout({
  ...
  backHref,
  children
}: Readonly<{ title: string; description: string; backHref: string; children: ReactNode }>) {
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link href={backHref as never} className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-muted/60">
...
export function InviteUserPage() {
  ...
  const [teamMembers, setTeamMembers] = useState<Array<{ id: string; fullName: string; email: string; role: string; status: string }>>([]);
  const form = useForm<{
    firstName: string;
    lastName: string;
    email: string;
  ...
  useEffect(() => {
    async function fetchMembers() {
      try {
        ...
        setTeamMembers((data.content ?? []).map((item) => ({
          id: item.id,
  ...
  const filteredMembers = useMemo(() => {
    ...
    return teamMembers.filter((member) => [member.fullName, member.email, member.role].some((value) => value.toLowerCase().includes(q)));
  }, [search, teamMembers]);
  ...
  return (
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><UserRoundPlus className="h-5 w-5" /> Personal Information</CardTitle>
            <CardDescription>Capture the core identity and contact details.</CardDescription>
          </CardHeader>
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="First name">
                  <Input {...form.register('firstName', { required: true })} placeholder="Elena" className="h-11 rounded-xl" />
                </Field>
                <Field label="Last name">
                  <Input {...form.register('lastName', { required: true })} placeholder="Vance" className="h-11 rounded-xl" />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="Phone number">
                  <Input {...form.register('phone')} placeholder="+1 (555) 000-0000" className="h-11 rounded-xl" />
                </Field>
              ...
              <Card className="border-muted/60 bg-muted/20">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base"><Shield className="h-4 w-4" /> Professional Credentials</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-2">
                    <Field label="NPI Number">
                      <Input placeholder="10-digit NPI" className="h-11 rounded-xl" />
                    </Field>
                    <Field label="Medical license number">
                      <Input {...form.register('licenseNumber')} placeholder="ST-12345678" className="h-11 rounded-xl" />
                    </Field>
                  </div>
                  <Field label="Primary specialty">
                    <Input {...form.register('specialization')} placeholder="Cardiology" className="h-11 rounded-xl" />
                  </Field>
                </CardContent>
              </Card>
                ...
                <CardContent className="grid gap-3 md:grid-cols-2">
                  {[
                    ...
                    <button
                      key={item.value}
                      type="button"
                      onClick={() => form.setValue('role', item.value as UserRole)}
              ...
              <div className="flex gap-3">
                <Button type="button" variant="outline" className="rounded-2xl" onClick={() => router.back()}>Cancel</Button>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Mail className="h-4 w-4" /> Profile Photo</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">Upload a professional high-resolution headshot for the patient portal.</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><BadgeCheck className="h-4 w-4" /> Security Tier</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between rounded-2xl bg-muted/50 p-4">
                <div>
                  <p className="font-medium">HIPAA Compliance</p>
                  <p className="text-xs text-muted-foreground">Strict PHI visibility</p>
                </div>
              ...
              <div className="flex items-center justify-between rounded-2xl bg-muted/50 p-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Manage global settings</p>
                </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Search className="h-4 w-4" /> Team Lookup</CardTitle>
              <CardDescription>Search the live roster before assigning a new member.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search members..." className="h-11 rounded-xl" />
...
export function AddDoctorPage() {
  const router = useRouter();
  const [status, setStatus] = useState<string | null>(null);
  const form = useForm({ defaultValues: { fullName: '', email: '', password: '', phone: '', specialization: '', licenseNumber: '', yearsOfExperience: '', qualifications: '', consultationFee: '', bio: '' } });
    ...
    >
      <Card className="max-w-3xl">
        <CardHeader>
          ...
          <CardDescription>Collect the core onboarding details used by the doctor registry.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <StatusBanner status={status} />
          <form
          ...
          >
            ...
            <Field label="Years of experience"><Input {...form.register('yearsOfExperience')} type="number" min="0" className="h-11 rounded-xl" placeholder="8" /></Field>
            <Field label="Consultation fee"><Input {...form.register('consultationFee')} className="h-11 rounded-xl" placeholder="150" /></Field>
            <Field label="Qualifications" ><Input {...form.register('qualifications')} className="h-11 rounded-xl md:col-span-2" placeholder="MBBS, MD" /></Field>
            <Field label="Bio"><Input {...form.register('bio')} className="h-11 rounded-xl md:col-span-2" placeholder="Short professional summary" /></Field>
            <Button type="submit" className="md:col-span-2 rounded-2xl" disabled={doctorMutation.isPending}>
...
export function ImportPatientsPage() {
  const router = useRouter();
  const [status, setStatus] = useState<string | null>(null);
  const form = useForm({ defaultValues: { fullName: '', email: '', password: '', phone: '', dateOfBirth: '', gender: '', medicalHistory: '', bloodType: '', allergies: '', emergencyContactName: '', emergencyContactPhone: '' } });
    ...
    >
      <Card className="max-w-3xl">
        <CardHeader>
          ...
          <CardDescription>Capture the core demographics and emergency contacts.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <StatusBanner status={status} />
          <form
          ...
          >
            ...
            <Field label="Blood type"><Input {...form.register('bloodType')} className="h-11 rounded-xl" placeholder="O+" /></Field>
            <Field label="Allergies"><Input {...form.register('allergies')} className="h-11 rounded-xl" placeholder="Penicillin" /></Field>
            <Field label="Emergency contact name"><Input {...form.register('emergencyContactName')} className="h-11 rounded-xl" placeholder="Fatima Yusuf" /></Field>
            <Field label="Emergency contact phone"><Input {...form.register('emergencyContactPhone')} className="h-11 rounded-xl" placeholder="+1 555 0300" /></Field>
            <Field label="Medical history"><Input {...form.register('medicalHistory')} className="h-11 rounded-xl md:col-span-2" placeholder="Hypertension, asthma" /></Field>
            <Button type="submit" className="md:col-span-2 rounded-2xl" disabled={patientMutation.isPending}>
...
export function CreateAppointmentPage() {
  const router = useRouter();
  const [status, setStatus] = useState<string | null>(null);
  const [selectedPatient, setSelectedPatient] = useState<{ id: string; fullName: string; bloodType: string; age: string } | null>(null);
  ...
  const filteredPatients = patientSearch ? patients.filter(p => p.fullName.toLowerCase().includes(patientSearch.toLowerCase())) : [];
    ...
    >
      <Card className="max-w-2xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Fill in the details to schedule a clinical visit.</CardDescription>
            </div>
            <Link href="#" className="text-sm text-teal-600 hover:text-teal-700 font-medium">+ New Patient</Link>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Patient Information */}
          <div>
            <label className="text-sm font-semibold text-slate-900 uppercase tracking-tight">Patient Information</label>
              ...
              {selectedPatient && (
                <div className="flex gap-3 rounded-2xl border border-border bg-muted/50 p-4">
                  <div className="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center flex-shrink-0">
                    <span className="text-sm font-semibold text-teal-700">{selectedPatient.fullName.charAt(0)}</span>
                  </div>
                  <div className="flex-1">
                    <p className="font-medium text-sm">{selectedPatient.fullName}</p>
                    <p className="text-xs text-muted-foreground">ID: CC-9982 • Age: {selectedPatient.age} • {selectedPatient.bloodType} Blood Type</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              <label className="text-sm font-semibold text-slate-900 uppercase tracking-tight">Department</label>
              <select
                value={department}
            ...
            <div>
              <label className="text-sm font-semibold text-slate-900 uppercase tracking-tight">Appointment Type</label>
          ...
          {/* Scheduling */}
          <div>
            <label className="text-sm font-semibold text-slate-900 uppercase tracking-tight">Scheduling</label>
            ...
            {hasConflict && (
              <div className="mt-4 rounded-2xl border-2 border-red-200 bg-red-50 p-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-red-900 text-sm">Scheduling Conflict</p>
                    <p className="text-red-800 text-sm mt-1">Dr. Smith has a "Surgery" block starting at 10:00 AM on Oct 24. This consultation might overlap.</p>
                    <button className="text-red-700 text-sm font-medium mt-2 hover:underline">SEE AVAILABLE SLOTS</button>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Doctor Availability */}
          <div>
            <label className="text-sm font-semibold text-slate-900 uppercase tracking-tight">Doctor Availability</label>
            <div className="mt-3">
              {doctors.slice(0, 1).map((doctor) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">Senior Neurologist • 12 Slots Open</p>
                  </div>
                  <Badge variant="outline" className="bg-teal-50 text-teal-700 border-teal-200">AVAILABLE</Badge>
                </label>
              ))}
            </div>
          </div>
